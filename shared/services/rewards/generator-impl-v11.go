...
type treeGeneratorImpl_v11 struct {
	...
	minipoolValidatorIndexMap    map[uint64]*MinipoolInfo
	megapoolValidatorIndexMap    map[uint64]*MegapoolInfo
...
func (r *treeGeneratorImpl_v11) fetchEpoch(getDuties bool, epoch uint64) (*epochState, error) {
...
func (r *treeGeneratorImpl_v11) fetchEpochs(startEpoch uint64, endEpoch uint64, errChan chan error, startTime time.Time) {
...
// Process an epoch, optionally getting the duties for all eligible minipools in it and checking each one's attestation performance
func (r *treeGeneratorImpl_v11) processEpoch(es *epochState) {
	...
	for i := uint64(0); i < r.slotsPerEpoch; i++ {
		...
		if len(es.attestations[i]) > 0 {
			...
			r.checkAttestations(es.attestations[i])
		}
	}
	...
	for slot, withdrawals := range es.withdrawals {
		...
		for _, withdrawal := range withdrawals {
			...
			mpi, exists := r.minipoolValidatorIndexMap[indexUint]
			if !exists {
				continue
			}
...
func (r *treeGeneratorImpl_v11) createMegapoolIndexMap() error {
	...
	r.megapoolValidatorIndexMap = map[uint64]*MegapoolInfo{}
	for _, details := range r.nodeDetails {
		...
		for _, validatorInfo := range details.Megapool.Validators {
			...
			default:
				...
				validatorInfo.Index = idx
				r.megapoolValidatorIndexMap[idx] = details.Megapool
...
func (r *treeGeneratorImpl_v11) createMinipoolIndexMap() error {
	...
	r.minipoolValidatorIndexMap = map[uint64]*MinipoolInfo{}
	for _, details := range r.nodeDetails {
		if !details.IsEligible {
			continue
		}
		for _, minipoolInfo := range details.Minipools {
			...
			default:
				...
				r.minipoolValidatorIndexMap[idx] = minipoolInfo
			...
			wg.Go(func() error {
				...
				if nativeNodeDetails.MegapoolDeployed {
					...
					mpInfo := &MegapoolInfo{
						...
						ValidatorIndexMap:    make(map[uint64]*MegapoolValidatorInfo),
